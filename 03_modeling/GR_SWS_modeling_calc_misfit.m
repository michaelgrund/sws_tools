function GR_SWS_modeling_calc_misfit(~,~)
%
    ...
    for kk=1:length(meas_phiSC)
        ...
        res_dt(kk,1)=abs(find_theo_dt-meas_dtSC(kk,2));
         ...
         if strcmp(curr_mod_type,'two_layers')
          ...
          BEST_models(count_mods).phi=model_out(ii).mod_paras.phi_in;
          BEST_models(count_mods).dt=model_out(ii).mod_paras.dt_in;
         ...
         elseif strcmp(curr_mod_type,'dipping')
          ...
          BEST_models(count_mods).downdipdir=model_out(ii).mod_paras.downdipdir;
          BEST_models(count_mods).dip=model_out(ii).mod_paras.dip;
          BEST_models(count_mods).thick=model_out(ii).mod_paras.thick; 
