...
%
%    (a) [SPLITS,NULLS]=SWS_modeling_read_evstruct => gives only the full structs 
%        (e.g. for further analysis)
%    (b) SWS_modeling_read_evstruct('stereo') => allows to directly generate (single) 
%        stereoplots for the analyzed stations via futher user input
%    (c) SWS_modeling_read_evstruct('stereoall') => automatically generate
%        stereoplots for all 266 seismic stations
%    (d) SWS_modeling_read_evstruct('histo') => generates a histogram showing the 
