function BEST_models_sort=SWS_modelling_calc_misfit_DIPPING_layer_4_systematic(SAVEDIR,MODELLstruct,dir_res_split,dir_res_nulls,dir_res_stack)
%
...
%clearvars -except structsnew model_out MODELLstruct
...
if ~isfield(model_out,'fast_eff') && ~isfield(model_out,'tlag_eff') 
    error('Required fields <phi_eff> & <dt_eff> do not exist in variable <model_out> (synthetic models)! Check input struct!')
end
...
for ii=1:length(model_out)
    ...
    curr_mod_phi=model_out(ii).fast_eff;
    curr_mod_dt=model_out(ii).tlag_eff;
...
%     end
    ...
    for kk=1:length(meas_phiSC)
        ...
        res_dt(kk,1)=abs(find_theo_dt-meas_dtSC(kk,2));
          ...
          BEST_models(count_mods).downdipdir=model_out(ii).downdipdir;
          BEST_models(count_mods).dip=model_out(ii).dip;
          BEST_models(count_mods).thick=model_out(ii).thick; 
          
          BEST_models(count_mods).fast_eff4plot=model_out(ii).fast_eff4plot;
          BEST_models(count_mods).tlag_eff4plot=model_out(ii).tlag_eff4plot; 
          BEST_models(count_mods).azi4plot=model_out(ii).azi4plot; 
          
          BEST_models(count_mods).modrange_low=modrange_low; 
          BEST_models(count_mods).modrange_upp=modrange_upp; 

          BEST_models(ii).RMS_phi=sqrt(sum(res_phi.^2)/length(meas_phiSC));
          BEST_models(ii).RMS_dt=sqrt(sum(res_dt.^2)/length(meas_phiSC));
          
          BEST_models(ii).staname=staname_split;
...
if wformat==1
 ...
 SWS_modelling_PLOT_results_vert(BAZ,BEST_models_sort,plot_mod_max,...
    ...
    color_edge_null,myfontsize,fonsize_subletters,modrange_col,modrange_edcol)
