function [hndl, marker, filename] = SWS_modelling_stereoplot_THEO_dipping_4_systematic_modelling(BEST_models_sort)
...
%%% old: load single parameters for a model search 
% load tlag_eff.mat
% load fast_eff.mat
% load azi.mat
% load downdipdir.mat
...
%==========================================================
%%% find input vlaues in MODELLstruct
...
% indexvals=find([MODELLstruct.downdipdir]==downdipdir & [MODELLstruct.dip]==dips & [MODELLstruct.thick]==thickness);
% 
% tlag_eff=MODELLstruct(indexvals).tlag_eff;
% fast_eff=MODELLstruct(indexvals).fast_eff;
% azi=MODELLstruct(indexvals).azi4plot;
% downdipdir=MODELLstruct(indexvals).downdipdir;

%indexvals=find([MODELLstruct.downdipdir]==downdipdir & [MODELLstruct.dip]==dips & [MODELLstruct.thick]==thickness);

usenum=1;

tlag_eff=BEST_models_sort(usenum).tlag_eff4plot;
fast_eff=BEST_models_sort(usenum).fast_eff4plot;
azi=BEST_models_sort(usenum).azi4plot;
downdipdir=BEST_models_sort(usenum).downdipdir;
dips=BEST_models_sort(usenum).dip;
...
if (BEST_models_sort(1).modrange_low ~=0 || BEST_models_sort(1).modrange_upp~=360)
    
    % first plot whole range in gray as botom layer
    startwedge=0;
    endwedge=360;
    P=plot_arc3D(deg2rad(startwedge-90),deg2rad(endwedge-90),0,0,0.2635,colfill);

    % then plot modelled range again on top in white ;)
    startwedge=BEST_models_sort(1).modrange_low;
    endwedge=BEST_models_sort(1).modrange_upp;
    P2=plot_arc3D(deg2rad(startwedge-90),deg2rad(endwedge-90),0,0,0.2635,[255.99999999 	255.99999999 	255.99999999]./256);
...
text(startval,.245,'2 s', 'HorizontalAlignment', 'center','fontsize',fontsize_leg)
   ...
   %cbpos(1) = 0.38+cbpos(1); compared to "measured" stereoplot the cb is slightly shifted
...
% 
% if dips==0
%       plot([-startval+xdirset-lengthbar/2 Ende(1)],[ydirset Ende(2)],'--','linewidth',linew-1,'color','b')
% else
%        plot([-startval+xdirset-lengthbar/2 Ende(1)],[ydirset+0.03*scaleddist Ende(2)+0.03*scaleddist],'--','linewidth',linew-1,'color','b')
% end
...
%fontsize_leg=14;
%text(-startval+0.105-lengthbar/2,-.276,[num2str(dips) '\circ'], 'HorizontalAlignment', 'left','fontsize',fontsize_leg,'color','k')
...
if colordisc==1 && savenamevar==1
    filename=['PLOT_modelling_DIPPING_' BEST_models_sort(1).staname '_THEO_stereo_dipdir' num2str(BEST_models_sort(1).downdipdir)...
        '_dip' num2str(dips) '_thick' num2str(BEST_models_sort(1).thick)];
